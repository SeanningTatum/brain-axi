...
// Line-anchored, and every fenced code block is stripped first: an unanchored
// substring search matched `**Verdict**:` inside a quoted example or a fence,
// so a doc could show what a verdict looks like and be scored on the sample.
const VERDICT_LINE_ALL = /^[ \t]*(?:[-*][ \t]*)?\*\*Verdict\*\*:[ \t]*(.+)$/gm;
const FENCED_BLOCK = /^[ \t]*(`{3,}|~{3,})[\s\S]*?^[ \t]*\1[ \t]*$/gm;
...
export function parseVerdictDetail(content) {
  // Strip fenced blocks so a documented example never scores as the verdict.
  const prose = String(content || "").replace(FENCED_BLOCK, "");
  const values = [];
  ...
  // Two verdicts is not a verdict. Silently taking the first let an amended doc
  // keep a stale PASS above a later FAIL.
  if (values.length > 1)
    return { verdict: "unknown", hasLine: true, form: "ambiguous", count: values.length };
  ...
  if (lead) {
    ...
    // An emoji that contradicts its word is ambiguous, not a tie to break.
    if (emoji && VERDICT_EMOJI[emoji] !== word)
      return { verdict: "unknown", hasLine: true, form: "conflicting" };
    return { verdict: word, hasLine: true, form: emoji ? "emoji" : "word" };
  }
  const emojiOnly = value.match(VERDICT_EMOJI_ONLY);
  if (emojiOnly) return { verdict: VERDICT_EMOJI[emojiOnly[1]], hasLine: true, form: "emoji" };

  return { verdict: "unknown", hasLine: true, form: "none" };
}
