...
export function loadRuns(brain, name) {
  ...
  rows.forEach((r, i) => {
    ...
    if (typeof r.ts !== "string" || !r.ts.trim()) {
      // Flagged, and excluded: a row with no timestamp cannot order the
      // history, feed `last_run`, or serve as a gate's prior run.
      errors.push(`${at} needs a string "ts"`);
      return;
    }
    runs.push(r);
  });
...
export function suiteRows(brain) {
  return listSuiteNames(brain).map((name) => {
    ...
    if (!run) state = "never-run";
      ...
      score !== null &&
      prevScore !== null &&
      score < prevScore - (suite.thresholds?.aggregate_tolerance ?? 0.02)
    )
      state = "regressed";
    return {
      suite: name,
...
export function scoreRun(cases, resultCases) {
  ...
  for (const r of resultCases) {
    // A runner reporting the same id twice silently discarded one verdict —
    // surface it instead: the run is marked partial like unknown ids are.
...
export function freezeCase(brain, name, id, frozen = true) {
  ...
  const out = lines.map((line) => {
    ...
    // `null`/`[...]`/"str" parse fine but are not cases — leave the line
    // untouched rather than crashing on `.id` (loadCases flags them already).
...
// Returns [] when the repo has no eval suites — the subsystem is optional and
// its absence (including a bare scaffolded evals/index.md) is not a failure.
