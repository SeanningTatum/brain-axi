...
Three of those sections are conditional. Each names the playbook that owns it,
because the standard is too long to inline here and too important to guess at:
...
placeholder for having thought about one.
    ...
    FIXED: "An agent that runs \`brain features set-status\` on a nonexistent
    slug gets \`error: feature not found\` and nothing else — no next step,
    no way to tell whether it mistyped the slug or the feature was renamed.
    That agent's job is to self-correct without escalating to a human;
    today it can't, so it either guesses or stops."
...
- **Reference-image pin-drop (the escape hatch from section 1).** Applies
  ...
  size). Hovering shows the same "⤢" affordance as a diagram; clicking it —
  in EITHER mode — posts the image up to the review chrome (the artifact
  iframe can't open the lightbox itself), which opens the shared
