...
// Per-round artifact snapshots (vN.html) written by recordReviewRound. These
// are the frozen copies a reviewer actually approved — the live `file` path
// keeps changing as the agent edits it, so anything checking "does the shipped
// thing match what was approved" has to read the snapshot, not the file.
function listSnapshots(brain, dir) {
  ...
  return names
    ...
    .filter((e) => e.m)
    .map((e) => ({ round: Number(e.m[1]), path: path.relative(brain, path.join(dir, e.name)) }))
