...
// The generated features/index.md table. Lives here, beside listVerifications
// and the drift check that validates its output, so there is exactly ONE
// definition of the format. It used to sit in bin/brain.js, where the test
// suite could not import it without executing the CLI's dispatch — so the
// suite hand-copied it, and a column added to the real generator would have
// left every drift fixture silently asserting the retired format.
export function buildFeaturesTable(brain, list) {
...
export function brainCheck(brain, opts = {}) {
  ...
  // SCOPE — the difference between an audit and a gate.
  //
  //   unscoped (`brain check`)      : whole brain. Report all debt; that IS the report.
  //   scoped   (`ship`, `set-status`): one feature. Refuse only what concerns IT.
  //
  // Scoping narrows every per-feature row: doc paths, dependency refs, verdict
  // readability, raw HTML, image links, and plans. It started life as
  // `strictScope`, covering only the two strict rows — which left the other
  // seven able to deadlock the gate exactly the way strict once did: one stray
  // <div> in an unrelated legacy doc refused every ship in the repo, naming a
  // file the shipper never touched. `strictScope` is still accepted as the old
  // name so an out-of-tree caller does not break silently (read-compat).
  const scope = opts.scope || opts.strictScope || null;
  const inScope = (slug) => !scope || slug === scope;
  try {
    ...
    const scopedFeatures = features.filter((f) => inScope(f.slug));
    const missingDocs = scopedFeatures
      .filter((f) => !resolveFeatureDocPath(brain, f))
      .map((f) => f.slug || f.id || "?");
    checks.push({
      check: "every feature doc path resolves",
      status: missingDocs.length ? "fail" : "pass",
      detail: missingDocs.length
        ? `missing doc for: ${missingDocs.join(", ")}`
        : scope
          ? `feature doc resolves for ${scope}`
          : `${features.length} feature doc(s) checked`,
    });
    ...
    checks.push({
      ...
      detail: badDeps.length
        ? badDeps.join(", ")
        : scope
          ? `all dependency refs resolve for ${scope}`
    ...
    const reviewBad = [];
    for (const { root, feature } of planRoots(brain)) {
      // A malformed plan under an unrelated feature — or in the unattributable
      // legacy pool — is real debt but is not this feature's problem, so it must
      // not refuse its ship.
      if (scope && feature !== scope) continue;
      if (!fs.existsSync(root)) continue;
      for (const slug of fs.readdirSync(root)) {
        const dir = path.join(root, slug);
        const mp = metaPathIn(dir);
    ...
    for (const slug of listFeatureSlugs(brain)) {
      ...
      for (const f of fs.readdirSync(dir)) {
        ...
        // Raw HTML lets the rendered doc and the parsed doc disagree — a verdict
        // in <details> renders collapsed, one in <div> renders as literal
        // asterisks. Banning it removes the ambiguity instead of trying to parse
        // it, which is a game with no last move.
        const stray = findRawHtml(content);
        if (stray.length)
          htmlBad.push(`${rel}: raw HTML not allowed (${stray.join(", ")}) — only the brain:verification receipt may be HTML`);
    ...
    checks.push({
      check: "verification docs contain no raw HTML",
      status: htmlBad.length ? "fail" : "pass",
      detail: htmlBad.length ? htmlBad.join(" | ") : `${verCount} verification doc(s) checked`,
    });
    checks.push({
    ...
    if (opts.strict) {
      ...
      // SCOPE (see the `scope` note at the top of brainCheck) limits the proof
      // requirement to one slug.
      //
      ...
      // The exemption key needs its OWN integrity check, or it is just a way to
      // turn the gate off: adding a slug exempted it unconditionally, so new work
      // could ship with zero verification by listing itself. Validated against
      // the ON-DISK list, never the projection — otherwise a ship could exempt
      // itself in the same breath.
      let onDisk = null;
      try {
        onDisk = JSON.parse(fs.readFileSync(flPath, "utf8"));
      } catch {
        onDisk = null;
      }
      const diskBySlug = new Map(
        (onDisk && Array.isArray(onDisk.features) ? onDisk.features : []).map((f) => [f.slug, f])
      );
      const badGrandfather = [];
      for (const slug of grandfathered) {
        const entry = diskBySlug.get(slug);
        if (!entry) {
          badGrandfather.push(`${slug}: not a known feature`);
        } else if (entry.status !== "shipped") {
          // A non-shipped entry cannot be "legacy debt" — the template's own list
          // carried a `cut` feature, which nothing noticed.
          badGrandfather.push(`${slug}: status "${entry.status}", not shipped — the list is for features that shipped BEFORE the invariant`);
        }
      }
      // Emit the row whenever a list exists, pass OR fail: a silent pass gave no
      // evidence the list had been validated at all, and the outstanding debt
      // should be visible on every run rather than only when something is wrong.
      if (grandfathered.size)
        checks.push({
          check: "strict grandfather list is legitimate",
          status: badGrandfather.length ? "fail" : "pass",
          detail: badGrandfather.length
            ? badGrandfather.join("; ")
            : `${grandfathered.size} legacy slug(s) exempt, all shipped: ${[...grandfathered].join(", ")}`,
        });
      ...
      const scopedNoop = Boolean(scope) && shipped.length === 0;
      // Ambient strict with NOTHING left to evaluate is not a pass. Every
      // shipped feature being grandfathered is the state this gate exists to
      // report, and calling it "pass" printed a green tick over the empty set —
      // the commit that promoted strict to a gate made it a gate over nothing.
      // `skip` keeps the exit code 0 (the debt is acknowledged, not failing)
      // while making the zero-coverage visible wherever the row is rendered.
      const shippedTotal = features.filter((f) => f.status === "shipped").length;
      const vacuous = !scope && shipped.length === 0 && shippedTotal > 0;
      if (vacuous) {
        const debt = `${shippedTotal} shipped feature(s), ${grandfathered.size} on policy.strict_grandfathered — 0 evaluated`;
        checks.push({
          check: "every shipped feature has a PASS verification",
          status: "skip",
          detail: `${debt}. Close the debt: verify a flow, add a receipt, then remove its slug from the list`,
        });
        checks.push({
