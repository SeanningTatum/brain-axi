...
export function brainCheck(brain, opts = {}) {
  ...
  try {
    ...
    const drift = [];
    let indexRows = 0;
    if (fs.existsSync(indexPath) && features.length) {
      let indexText = "";
      try {
      ...
      for (const line of indexText.split("\n")) {
        ...
        // Attribute the row by its doc link, not by loose slug mention, so prose
        // elsewhere in the file cannot be misread as a status row.
        const feat = features.find(
          (f) => f.slug && (line.includes(`${f.slug}/${f.slug}.md`) || line.includes(`(${f.slug}.md)`))
        );
        if (!feat) continue;
        const found = STATUSES.filter((s) => new RegExp(`(^|[^a-z-])${s}([^a-z-]|$)`).test(line));
        if (found.length !== 1) continue; // ambiguous or status-free row — not ours to judge
        indexRows++;
        if (found[0] !== feat.status)
