...
window.addEventListener("message", (e) => {
  ...
  switch (data.type) {
    ...
    case "brain:queuePrompt":
      ...
      break;
    case "brain:openReferenceImage":
      openReferenceLightbox(data);
      break;
...
// ---- reference-image pin-drop lightbox -------------------------------------
// The artifact SDK posts `brain:openReferenceImage` when the reviewer clicks a
// reference <img> in the artifact (it can't open the lightbox itself — the
// lightbox lives here, in the chrome). Reuse the SAME BrainLightbox the
// execution-screenshot rail uses; committed pins land in the composer queue as
// tag `reference-image`, delivered on Send like every other annotation.

function openReferenceLightbox(data) {
  if (!data || !data.url) return;
  // Guard: a stale cached chrome may lack the lightbox — fall back to a new tab.
  if (!window.BrainLightbox) {
    window.open(data.url, "_blank", "noopener");
    return;
  }
  // `rel` keys pins to this image inside the lightbox; the selector is the
  // durable locator (the src may be a data: URI), so prefer it.
  const ref = data.selector || data.url;
  const payload = [{ url: data.url, caption: data.caption || data.alt || "reference image", rel: ref }];
  window.BrainLightbox.open(payload, 0, {
    onAnnotate: (annotation) => queueReferenceAnnotation(annotation, data),
    annotatedCta: {
      text: "Queued in the composer — press Send to deliver your feedback to the agent.",
      actionLabel: "Go to Send",
      action: (ctx) => {
        ctx.close();
...
// Push a committed reference-image pin into the shared composer queue. Mirrors
// queueScreenshotAnnotation (renderExecution) but tag `reference-image` and a
// selector-anchored target — handleQueuePrompt is synchronous, so BrainLightbox
// treats the return as an immediate commit. queueKey (refimg:selector:x:y)
// coalesces repeated pins dropped on the same spot.
function queueReferenceAnnotation(annotation, data) {
  const caption = annotation.caption || (data && (data.caption || data.alt)) || "reference image";
  const selector = annotation.shot || (data && data.selector) || "";
  const coord =
    annotation.x != null && annotation.y != null ? " (pin at " + annotation.x + "%, " + annotation.y + "%)" : "";
  handleQueuePrompt({
    tag: "reference-image",
    prompt: 'Reference image "' + caption + '": ' + annotation.note + coord,
    text: caption,
    target: {
    ...
    queueKey: "refimg:" + selector + ":" + annotation.x + ":" + annotation.y
  });
}
      ...
      img.addEventListener("click", () => {
        ...
        if (window.BrainLightbox) {
          window.BrainLightbox.open(payload, idx, {
            annotations,
            onAnnotate: queueScreenshotAnnotation,
            annotatedCta: {
              text: "Queued in the composer — press Send to deliver your feedback to the agent.",
              actionLabel: "Go to Send",
              action: (ctx) => {
...
async function fetchHealth() {
  ...
  try {
    ...
    // "skip" is a check that had nothing to evaluate (e.g. strict with every
    // shipped feature grandfathered). It is not a failure — but it is not a
    // pass either, so it must not be counted as one in the ok-line total.
    const failing = checks.filter((c) => c.status !== "pass" && c.status !== "skip");
    if (!failing.length) {
      const line = document.createElement("div");
